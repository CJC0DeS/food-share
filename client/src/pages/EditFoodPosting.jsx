...
export default function EditFoodPosting() {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      ...
      const res = await fetch(`/api/foodpost/get/${postId}`);
  ...
  const handleImagesSubmit = async (e) => {
    e.preventDefault();
    console.log(formData.imageUrl.length);
    setImageUploadingStatus(true);
    if (images.length > 0 && images.length < 7 - formData.imageUrl.length) {
      const promises = [];
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await fetch(`/api/foodpost/update/${params.postId}`, {
        method: "POST",
        headers: {
    ...
    <main className=" p-3 max-w-8xl mx-auto">
      ...
      <div className="mt-8">
        <button
          ...
          disabled={loading || imageUploadingStatus || !isContentChanged}
          className=" mx-auto flex flex-row justify-center align-center p-3 bg-lime-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-60 w-64"
        >
